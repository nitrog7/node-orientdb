...
exports.create = function(config) {
  ...
  var command = 'CREATE VERTEX', promise, cluster, className, attributes;
 ...
 * @promise {Number}       The number of vertices deleted.
 */
exports.delete = function(config) {
  var rid = config['@rid'] || RID.parse(config), command;
  ...
  return this.query(command).then(function(response) {
    return +response || 0;
  });
};
