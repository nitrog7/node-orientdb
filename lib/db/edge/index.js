...
function createEdge(db, config, from, to) {
  var command = 'CREATE EDGE', className, attributes;
 ...
 * @param   {Mixed}  from   The vertex to start from.
 * @param   {Mixed}  to     The vertex to end at.
 * @promise {Number}       The number of deleted edges.
 */
var deleteEdge = function(db, config, from, to) {
  var command = 'DELETE EDGE', className = config ? edgeConfig(config)[0] : false;
  ...
  return db.query(command).then(function(response) {
    return +response || 0;
  });
};
 ...
 */
var edgeConfig = function(config) {
  var className = 'E';

  if(_.isString(config)) {
    className = config;
  } else if(config && _.isObject(config)) {
