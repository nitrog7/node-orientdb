...
CLI.prototype.run = function(argv) {
  ...
  return this.parseArgv(argv).bind(this).then(function(argv) {
    var command = argv._[0] || 'help', total, i;
...
CLI.prototype.createCommand = function(name, argv) {
  ...
  try {
    var Constructor = Command.extend(require(filename)), server = this.createServer(argv);
...
CLI.prototype.parseArgv = function(argv) {
  return this.declareArgv().bind(this).then(function() {
    ...
    return this.parseOptsFile(path.join(interim.cwd, 'orientdb.opts')).then(function(opts) {
...
CLI.prototype.declareArgv = function() {
  return this.list().then(function(names) {
    return yargs.usage('Usage: orientdb [OPTIONS] [' + names.join('|') + ']').options({
      d: {
        alias: 'cwd',
        default: process.cwd(),
...
CLI.prototype.list = function() {
  return fs.readdirAsync(path.join(__dirname, 'commands')).filter(function(filename) {
