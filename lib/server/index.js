...
Server.prototype.applySettings = function (options) {
  ...
  self.host       = options.host || options.hostname || 'localhost';
  self.port       = options.port || 2424;
  self.username   = options.username || 'root';
  self.password   = options.password || '';

  self.sessionId  = -1;
  self.ops        = new OperationQueue();
  self.ops.on('update-config', function (config) {
    this.logger.debug('updating config...');
    this.serverCluster = config;
  }.bind(this));

  self.configureLogger(options.logger || {});
};
...
Server.prototype.use = function(config) {
  ...
  if(_.isObject(config) && _.isUndefined(config.name)) {
    throw new errors.Config('Cannot use a database without a name.');
  }
