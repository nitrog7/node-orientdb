...
Operation.prototype.writeString = function(data) {
  ...
  this.writeOps.push([constants.BYTES_INT + len, function(buffer, offset) {
    buffer.fill(0, offset, offset + constants.BYTES_INT + len);
    buffer.writeInt32BE(len, offset);
...
Operation.prototype.canRead = function(type, buffer, offset) {
  ...
  switch(type) {
    ...
    case 'Short':
    case 'Record':
      return len>=offset + constants.BYTES_SHORT;
    case 'Long':
      return len>=offset + constants.BYTES_LONG;
    case 'Int':
    case 'Collection':
      return len>=offset + constants.BYTES_INT;
    case 'Bytes':
    case 'String':
      if(len<=offset + constants.BYTES_INT) {
        return false;
      } else {
        return len>=offset + constants.BYTES_INT + buffer.readInt32BE(offset);
      }
      break;
    default:
      return false;
...
Operation.prototype.parseBytes = function(buffer, offset, context, fieldName, reader) {
  ...
  if(len<0) {
    ...
    context[fieldName] = buffer.slice(offset, offset + len);
  }
  ...
  return len>0 ? len + constants.BYTES_INT : constants.BYTES_INT;
};
...
Operation.prototype.parsePushedData = function(buffer, offset, context, fieldName, reader) {
  var len = buffer.readInt32BE(offset),
    asString;
  offset += constants.BYTES_INT;
  asString = buffer.toString('utf8', offset, offset + len);
  ...
  return len + constants.BYTES_INT;
};