...
var deserializeValue = function(value) {
  ...
  var rid = RecordID.parse(value).toString();
  ...
  if(firstChar === '{') {
    ...
    for(var g = 0, len = values.length; g<len; g++) {
      values[g] = deserializeValue(values[g]);
    }
 ...
 */
var findCommaIndex = function(serialized) {
  var delimiters = [];

  for(var g = 0, len = serialized.length; g<len; g++) {
    var current = serialized[g];
    var prev = delimiters[delimiters.length - 1];
