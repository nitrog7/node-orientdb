 ...
 */
OperationQueue.prototype.add = function (operation, params) {
  if(_.isString(operation)) {
    operation = new operations[operation](params || {});
  }
  ...
  if (operation.id === 'REQUEST_DB_CLOSE') {
    deferred.resolve({});
  } else {
    this.items.push([operation, deferred]);
  }
...
OperationQueue.prototype.process = function (buffer, offset) {
  var parsed, result, status, item, op, deferred;
  offset = offset || 0;
